...
import { ensureHook, getUpdateUI } from "./hook";
import {
	SYMBOL_H,
	SYMBOL_TILES,
	SYMBOL_W,
...
import { config, type OverlayItem, saveConfig } from "./store";
...
export async function buildOverlayDataForChunkUnified(
	...
	const drawX =
		base.chunk1 * TILE_SIZE + base.posX + ov.offsetX - targetChunk1 * TILE_SIZE;
	const drawY =
		base.chunk2 * TILE_SIZE + base.posY + ov.offsetY - targetChunk2 * TILE_SIZE;
	...
	if (mode !== "minify") {
		...
		if (config.minifyStyle === "symbols") {
			...
			const finalImageData = outCtx.getImageData(
				finalIsect.x,
				finalIsect.y,
				finalIsect.w,
				finalIsect.h,
...
export async function composeTileUnified(
	...
	if (mode === "minify") {
		const scale =
			config.minifyStyle === "symbols" ? MINIFY_SCALE_SYMBOL : MINIFY_SCALE;
		...
		const hash =
			view.getUint32(0, true) ^ view.getUint32(view.byteLength - 4, true);
