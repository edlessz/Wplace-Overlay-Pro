...
import { type EV_ANCHOR_SET_Event, uid, uniqueName } from "../core/util";
...
function rebuildOverlayListUI() {
    ...
    `;
		const [radio, checkbox, nameDiv, trashBtn] = Array.from(item.children) as [
			HTMLInputElement,
			HTMLInputElement,
			HTMLDivElement,
			HTMLButtonElement,
...
async function importOverlayFromJSON(jsonText: string) {
	...
	alert(
		`Import finished. Imported: ${imported}${failed ? `, Failed: ${failed}` : ""}`,
	);
}
...
function addEventListeners(panel: HTMLDivElement) {
	...
	$("op-style-dots").addEventListener("change", () => {
		if (($("op-style-dots") as HTMLInputElement).checked) {
	...
	$("op-style-symbols").addEventListener("change", () => {
		if (($("op-style-symbols") as HTMLInputElement).checked) {
	...
	$("op-name").addEventListener("change", async (e: Event) => {
		const ov = getActiveOverlay();
		if (!ov) return;
		const desired =
			((e.target as HTMLInputElement).value || "").trim() || "Overlay";
		if (
	...
	$("op-fetch").addEventListener("click", async () => {
		...
		const url = ($("op-image-url") as HTMLInputElement).value.trim();
		if (!url) {
	...
	$("op-file-input").addEventListener("change", async (e: Event) => {
	...
	dropzone.addEventListener(
		"drop",
		async (e: Event & { dataTransfer: HTMLInputElement }) => {
	...
	$("op-opacity-slider").addEventListener("input", (e: Event) => {
		const ov = getActiveOverlay();
		if (!ov) return;
		ov.opacity = parseFloat((e.target as HTMLInputElement).value);
...
export function updateUI() {
	...
	panelEl.querySelectorAll(".op-tab-btn").forEach((btn) => {
		...
		if (
			mode === "above" &&
			(config.overlayMode === "above" || config.overlayMode === "behind")
		) {
			isActive = true;
		} else {
	...
	// --- Mode Settings ---
	const fullOverlaySettings = $("op-mode-settings").querySelector(
		'[data-setting="above"]',
	) as HTMLDivElement;
	const minifySettings = $("op-mode-settings").querySelector(
		'[data-setting="minify"]',
	) as HTMLDivElement;
	...
	($("op-style-dots") as HTMLInputElement).checked =
		config.minifyStyle === "dots";
	($("op-style-symbols") as HTMLInputElement).checked =
		config.minifyStyle === "symbols";
	...
	if (positioningBody)
		positioningBody.style.display = config.collapsePositioning
			? "none"
			: "block";
	if (positioningCz)
		positioningCz.textContent = config.collapsePositioning ? "▸" : "▾";
